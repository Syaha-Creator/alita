...
/// Remote data source untuk product API calls
...
class ProductRemoteDataSourceImpl implements ProductRemoteDataSource {
  ...
  Future<List<ProductModel>> fetchProductsWithFilter({
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        if (rawData is! List) {
          throw ServerException(
            "Data produk tidak ditemukan. Silakan coba lagi.",
          );
        }
        ...
        if (e.type == DioExceptionType.connectionTimeout ||
          ...
          if (retryCount < maxRetries) {
            ...
            final delaySeconds =
                retryCount * RetryDurations.retryDelay.inSeconds;
  ...
  Future<List<ProductModel>> _fetchWithFallback(
    ...
    try {
      String? token = await AuthService.getToken();
      if (token == null || token.isEmpty) {
        throw ServerException("Token tidak tersedia. Harap login ulang.");
      }

      int? userId = await AuthService.getCurrentUserId();
      if (userId == null) {
        throw ServerException("User ID tidak tersedia. Harap login ulang.");
      }
      ...
      if (response.statusCode == 401) {
        ...
        if (newToken != null) {
          ...
          throw ServerException("Token refresh failed. Harap login ulang.");
        }
      }
