...
public class FlutterSecureStoragePlugin implements MethodCallHandler, FlutterPlugin {
    ...
    class MethodRunner implements Runnable {
        ...
        @Override
        public void run() {
            // Guard against race condition: onDetachedFromEngine() can set secureStorage=null
            // while a MethodRunner task is still queued in the worker thread (Android 16+).
            if (secureStorage == null) {
                result.error("storage_unavailable", "Plugin detached from engine before operation could run", null);
                return;
            }
